...
def get_rescale_fn(dim, min_hz, max_hz):
    ...
    def stretch(time, stretch_factor):
        ...
        new_x = tfp.math.interp_regular_1d_grid(tf.range(0, length) - length / 2, x_ref_min=-new_length / 2,
                                                x_ref_max=new_length / 2, y_ref=time)
    ...
    def joint_random_rescale(x, y):
        ...
        """
        stretch_factor = tf.random.uniform((), tf.math.maximum(min_bpm / y, 0.75),
                                           tf.math.minimum(max_bpm / y, 1.25), dtype=tf.float32)
        spec, time = stretch_spec(x[0], stretch_factor, y), stretch(x[1], stretch_factor)
...
def corrupt_one_representation(x, y):
    ...
    freq, time = add_gaussian_noise(x[0], strength=s), add_gaussian_noise(x[1], strength=1 - s)
    return (freq, time), y
...
def add_augmentations(ds, args, rescale=True, add_noise=True):
    ...
    if rescale:
        ...
        ds = ds.map(rescale_fn, num_parallel_calls=tf.data.AUTOTUNE).prefetch(tf.data.AUTOTUNE)
    if add_noise:
        ds = ds.map(corrupt_one_representation, num_parallel_calls=tf.data.AUTOTUNE).prefetch(
            tf.data.AUTOTUNE)
    return ds
